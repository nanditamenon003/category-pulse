...
**The AI asks before it sees your data.** The chat runs on a low-cost AI provider, which is fine for simulated or dummy data but not for a store's real figures without approval. So on uploaded data it first explains, in plain words, that the figures it looks up are sent to that provider, and asks for a clear yes once per visit. Its tools and instructions are built from the uploaded store itself, and tools for data the store doesn't have are left out, so it says plainly when a question can't be answered. Uploaded data is read for the visitor's session only and isn't saved, and "Remove my data" clears it.
